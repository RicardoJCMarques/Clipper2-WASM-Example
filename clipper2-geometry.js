 ...
 * @see         {@link https://github.com/RicardoJCMarques/Clipper2-WASM-Example}
 *
...
class Clipper2Geometry {
    ...
    toClipper2Path(definition, options = {}) {
        ...
        switch (definition.type) {
            ...
            case 'strokes':
                return this.strokesToPath64(definition);
            case 'svg':
                return this.svgToPath64(definition, options);
            case 'pcb':
                return this.pcbToPath64(definition);
            default:
    ...
    /**
     * Convert geometry definition to Clipper2 Paths64
     */
    ...
    validatePolygon(coords) {
        ...
        const hasBasicSelfIntersection = this._checkBasicSelfIntersection(coords);
     ...
     */
    _checkBasicSelfIntersection(coords) {
        ...
        for (let i = 0; i < n; i++) {
            ...
            for (let j = i + 2; j < n; j++) {
                ...
                if (this._doSegmentsIntersect(a1, a2, b1, b2)) {
                    return true;
                }
            }
        }
     ...
     */
    _doSegmentsIntersect(p1, p2, p3, p4) {
     ...
     */
    coordinatesToPath64(coords, options = {}) {
        ...
        // Only fix winding if explicitly requested or default (true)
        // Set autoFixWinding: false for Area tests or raw data preservation
        if (options.autoFixWinding !== false) {
    ...
    ensureCounterClockwise(path) {
        // Use Clipper2's native Area function for robustness
        const area = this.core.clipper2.AreaPath64(path);

        // In Clipper2: Positive Area = Counter-Clockwise
        // If Area is negative (CW), reverse it
        if (area < 0) {
